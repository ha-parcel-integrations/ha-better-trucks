"""Sensor platform for the Better Trucks parcel tracker integration."""
...
from .coordinator import BetterTrucksCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: BetterTrucksConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Better Trucks sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        BetterTrucksIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            BetterTrucksParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(BetterTrucksNextDeliverySensor(coordinator, entry))
    entities.append(BetterTrucksDeliveredParcelsSensor(coordinator, entry))
    entities.append(BetterTrucksLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class BetterTrucksIncomingParcelsSensor(
    CoordinatorEntity[BetterTrucksCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: BetterTrucksCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                BetterTrucksParcelSensor(self.coordinator, self._entry, barcode)
...
class BetterTrucksParcelSensor(CoordinatorEntity[BetterTrucksCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Better Trucks parcel."""
    ...
    def __init__(
        self, coordinator: BetterTrucksCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class BetterTrucksNextDeliverySensor(
    CoordinatorEntity[BetterTrucksCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: BetterTrucksCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class BetterTrucksDeliveredParcelsSensor(
    CoordinatorEntity[BetterTrucksCoordinator], SensorEntity
):
    """Recently delivered tracked Better Trucks parcels."""
...
class BetterTrucksLastUpdateSensor(
    CoordinatorEntity[BetterTrucksCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Better Trucks was last polled successfully."""
