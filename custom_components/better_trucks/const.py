"""Constants for the Better Trucks parcel tracker integration."""
from enum import StrEnum

from homeassistant.const import Platform

DOMAIN = "better_trucks"
...
# Better Trucks provides: delivery_window (eta), url (tracking URL), history
# Better Trucks does NOT provide: weight, dimensions, pickup_point
CAPABILITIES = frozenset(
    {"delivery_window", "url", "history"}
)
...
# Better Trucks public tracking endpoint.
# - No authentication required (keyless, code-based carrier)
# - No rate limiting observed (10 requests with 3-request burst, no 429/Retry-After)
# - Response content type: application/json
# - Unknown tracking code: HTTP 200 with status="UNKNOWN" and tracking_history=[]
#   (NOT a 404; this is a normal pending state, not an error)
# - isFromExternalTracking=true parameter is sent to match consumer page behavior
#   (flag is inert but kept for consistency)
TRACKING_API_URL = "https://tracking.bettertrucks.com/api/tracking/{tracking_code}?isFromExternalTracking=true"
TRACKING_URL = "https://tracking.bettertrucks.com/track/{tracking_code}"
