"""Coordinator for the Better Trucks parcel tracker integration.
...
from .api import NEW_ISSUE_URL, BetterTrucksApiClient, BetterTrucksApiError
from .const import (
...
# Fires once, the first time this endpoint is ever seen to throttle — Gate A
# found no 429/Retry-After/RateLimit-* across ten requests including a
# three-request burst, so the first one is news worth reporting.
_warned_throttling = False


def _warn_throttling_once() -> None:
    global _warned_throttling
    if _warned_throttling:
        return
    _warned_throttling = True
    _LOGGER.warning(
        "Better Trucks returned HTTP 429 for the first time — this "
        "endpoint was believed unthrottled. Open an issue to help confirm "
        "the polling interval it actually needs: %s",
        NEW_ISSUE_URL,
    )
...
class BetterTrucksCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (BetterTrucksApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, BetterTrucksApiError)
                    and result.status_code == 429
                ):
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("Better Trucks fetch failed for %s: %s", code, result)
            ...
            if result is None:
                # Unknown code (UNKNOWN status or empty tracking_history), or not scanned yet.
                # Keep prior data if we have it, otherwise show a pending placeholder so the user still
                # sees the parcel they asked us to track.
                raws.append(self._raw_cache.get(code) or {"tracking_number": code})
                continue

            # The response's own tracking number can be missing on edge
            # payloads; fall back to the code we asked for so the sensor keeps
            # its key.
            result.setdefault("tracking_number", code)
            self._raw_cache[code] = result
            raws.append(result)

        if saw_429:
            _warn_throttling_once()
            ...
            raise UpdateFailed(
                "Better Trucks rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("Better Trucks unreachable for all tracked parcels")
