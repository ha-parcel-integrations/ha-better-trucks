"""Calendar platform for the Better Trucks parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Better Trucks deliveries calendar from a config entry."""
    async_add_entities([BetterTrucksDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class BetterTrucksDeliveriesCalendar(CoordinatorEntity[BetterTrucksCoordinator], CalendarEntity):
    """A read-only calendar of expected Better Trucks deliveries.
    ...
    def __init__(self, coordinator: BetterTrucksCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Better Trucks parcel")
            description_parts = [
