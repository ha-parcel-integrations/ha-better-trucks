# Automatic parcel tracking from e-mail (IMAP → Better Trucks)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Better Trucks tracking code, and register it with `better_trucks.track_parcel` — fully automatic, no extra custom component required.

Better Trucks is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `better_trucks.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ better_trucks.track_parcel
                                     │
...
- This integration, with the `better_trucks.track_parcel` action available (field `tracking_code`).
...
The regex matches the `BTS_` prefix followed by alphanumerics:

```
(?<![0-9A-Za-z])BTS_[0-9A-Za-z]{8,20}
```

The integration accepts any code matching `^BTS_[A-Z0-9]{8,20}$`. The AI fallback will surface any variant formats in the meantime.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@bettertrucks.com","subject":"Your parcel is on its way",
       "text":"Tracking code: BTS_12345678901","initial":true,"folder":"INBOX","username":"test"}'
```

Then `better_trucks.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
