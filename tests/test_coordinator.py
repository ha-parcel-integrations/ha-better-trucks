"""Tests for the Better Trucks coordinator: fetching, caching and events.
...
from custom_components.better_trucks import coordinator as coordinator_module
from custom_components.better_trucks.api import BetterTrucksApiError
...
from custom_components.better_trucks.coordinator import BetterTrucksCoordinator

from .payloads import ACTIVE_CODE, DELIVERED_CODE, active_sample, delivered_sample

OTHER_CODE = "BTS_888888888"
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["tracking_status"]["status"] = "INJECTED"  # Better Trucks uses INJECTED for in-transit
    sample["tracking_status"]["status_details"] = "Received in BT network"
    return sample
...
async def test_update_merges_multiple_parcels(hass):
    ...
    coordinator = BetterTrucksCoordinator(hass, client, entry)
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = BetterTrucksApiError("HTTP 500")
...
async def test_first_429_warns_once_then_raises_update_failed(hass, caplog):
    """The throttling WARNING (pre-1.0 obligation) fires once, before UpdateFailed."""
    from homeassistant.helpers.update_coordinator import UpdateFailed

    coordinator_module._warned_throttling = False
    entry = _entry_with([{CONF_TRACKING_CODE: DELIVERED_CODE}])
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.side_effect = BetterTrucksApiError(
    ...
    assert caplog.text.count("believed unthrottled") == 1
...
async def test_update_prunes_cache_for_untracked_parcels(hass):
    ...
    client.async_get_parcel.return_value = delivered_sample()
    coordinator = BetterTrucksCoordinator(hass, client, entry)
    coordinator._raw_cache["GONE"] = {"tracking_number": "GONE"}
...
async def test_fires_delivery_time_changed_event(hass):
    ...
    moved = active_sample()
    # Better Trucks uses 'eta' as a point-estimate field
    moved["eta"] = "2026-06-12T20:00:00Z"
    client.async_get_parcel.return_value = moved
    await coordinator._async_update_data()
    await hass.async_block_till_done()

    assert len(events) == 1
    assert events[0].data["old_planned_from"] == "2026-06-12T18:51:48+00:00"
    assert events[0].data["new_planned_from"] == "2026-06-12T20:00:00+00:00"
...
async def test_losing_the_eta_is_silent(hass):
    ...
    # Better Trucks: set eta to None — carrier lost the estimate; should be silent
    dropped["eta"] = None
